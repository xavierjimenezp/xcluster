...
class CNNSegmentation(object):
    ...
    def train_model(self):
        input_train = np.load(self.dataset_path + 'input_train_pre_f%s_'%self.freq + self.dataset + '.npz')['arr_0']
        input_val = np.load(self.dataset_path + 'input_val_pre_f%s_'%self.freq + self.dataset + '.npz')['arr_0']
        train_dataset, valid_dataset, test_dataset = self.npy_to_tfdata(batch_size=batch, buffer_size=1000)
    ...
    def evaluate_prediction(self, plot=True, plot_patch=True):
        ...
        with CustomObjectScope({'iou': losses.iou, 'f1': losses.f1, 'focal_tversky': losses.focal_tversky, 'dsc': losses.dsc,
                                'tp': losses.tp, 'tn': losses.tn, 'tversky_loss': losses.tversky_loss}):
        ...
        if plot == True:
            ...
            self.make_predictions(model, dataset, num=30)
